...
def visualization_2_cluster_spatial_distribution(X_scaled, Z, df_row, features, logger) -> None:
    ...
    for i in range(n_clusters):
        ...
        ax.scatter(
            cluster_data["chainage_km"],
            cluster_data["ndvi_mean"],
            c=str(gray_value),
            label=f"C{i}: {names[i]}",
            marker=markers[i],
            s=sizes[i],
    ...
    ax.axhline(
        y=0.7, color="#2980b9", linestyle="--", linewidth=1.5, alpha=0.7, label="NDVI threshold"
    )
    signalplot.tidy_axes(ax)
    plt.tight_layout()
    ...
    ax2.bar(
        range(n_clusters),
        cluster_profiles["bare_soil_fraction"],
        color="#ffffff",
        edgecolor="#2b2b2b",
        linewidth=1.5,
        alpha=0.9,
    ...
    ax3.bar(
        range(n_clusters),
        cluster_profiles["thermal_anomaly_score"],
        color="#ffffff",
        edgecolor="#2b2b2b",
        linewidth=1.5,
        alpha=0.9,
    ...
    logger.info("\nOperational Summary:")
    logger.info(
        f"  High-risk tiles (Clusters 1, 4): {len(df_row[df_row['cluster_id'].isin([1, 4])])}"
    )
    logger.info(
        f"  Standard monitoring (Clusters 0, 2, 3): {len(df_row[~df_row['cluster_id'].isin([1, 4])])}"
    )
